module _81_More_on_wrapper_types
// We discover that even lists can be wrapper types

// In the previous post, we looked at the concept of “wrapper types” and their relation 
// to computation expressions. In this post, we’ll investigate what types are suitable 
// for being wrapper types.

// What kinds of types can be wrapper types?
// If every computation expression must have an associated wrapper type, then what kinds 
// of type can be used as wrapper types? Are there any special constraints or limitations 
//that apply?
// There is one general rule, which is:
// Any type with a generic parameter can be used as a wrapper type
// So for example, you can use Option<T>, DbResult<T>, etc., as wrapper types, as we have 
// seen. And you can use wrapper types that restrict the type parameter, such as 
// Vector<int>.
...
// For example, we saw in an earlier example an attempt to do addition on strings, like 
// this: "1" + "2". Can’t we be clever and treat string as a wrapper type for int in this 
// case? That would be cool, yes?
// Let’s try. We can use the signatures of Bind and Return to guide our implementation.
// - Bind takes a tuple. The first part of the tuple is the wrapped type (string in this 
//   case), and the second part of the tuple is a function that takes an unwrapped type 
//   and converts it to a wrapped type. In this case, that would be int -> string.
...
// - But what happens if the bind function can’t unwrap a string, because it is not a 
//   valid number? In this case, the bind function must still return a wrapped type (a 
//   string), so we can just return a string such as “error”.
...
// Let’s say we give the workflow an input, unwrap it (with let!) and then immediately 
...
// Would this be a problem in practice? I don’t know. But I would avoid it and use a 
// different approach, like options, that are consistent in all cases.

// Rules for workflows that use wrapper types
// Here’s a question? What is the difference between these two code fragments, and should 
...
//    return! wrapped
//    } 
// The answer is no, they should not behave differently. The only difference is that in 
// the second example, the unwrapped value has been refactored away and the wrapped value 
// is returned directly.
// But as we just saw in the previous section, you can get inconsistencies if you are not 
// careful. So, any implementation you create should be sure to follow some standard 
// rules, which are:

// Rule 1: If you start with an unwrapped value, and then you wrap it (using return), 
// then unwrap it (using bind), you should always get back the original unwrapped value.

// This rule and the next are about not losing information as you wrap and unwrap the 
// values. Obviously, a sensible thing to ask, and required for refactoring to work as 
// expected.
...
// This rule is required for composition to behave properly, and again, “extraction” 
// refactoring will only work correctly if this is true.
// In general, you will get this for free if you follow some guidelines (which will be 
...
// But how can this be? There is no one-to-one correspondence between the wrapper type and 
// the unwrapped type!
// This is where the “wrapper type” analogy becomes a bit misleading. Instead, let’s go 
// back to thinking of bind as a way of connecting the output of one expression with the 
// input of another.
// As we have seen, the bind function “unwraps” the type, and applies the continuation 
// function to the unwrapped value. But there is nothing in the definition that says that 
// there has to be only one unwrapped value. There is no reason that we can’t apply the 
// continuation function to each item of the list in turn.
...
// required to have a certain signature. It takes an unwrapped type, but it produces a 
// wrapped type.
...
// the “list of lists” is no good. We need to turn them back into a simple “one-level” 
// list.
// But that is easy enough – there is a list module function that does just that, called 
// concat.
...
// - Bind applies the continuation function to each element of the passed in list, and 
//   then flattens the resulting list of lists into a one-level list. List.collect is a 
//   library function that does exactly that.
...
// And the results show that every element in the first collection has been combined 
...
// That’s quite amazing really. We have completely hidden the list enumeration logic, 
// leaving just the workflow itself.

// Syntactic sugar for “for”
// If we treat lists and sequences as a special case, we can add some nice syntactic sugar 
// to replace let! with something a bit more natural.
...
// It generally has exactly the same implementation as the normal Bind method, but is 
...
// Does the for element in collection do look familiar? It is very close to the from 
// element in collection ... syntax used by LINQ. And indeed LINQ uses basically the same 
// technique to convert from a query expression syntax like from element in collection ... 
...
// written a helpful blog post 
// http://codeblog.jonskeet.uk/2010/12/27/reimplementing-linq-to-objects-part-9-selectmany/
// explaining this.

// The identity “wrapper type”
// So we’ve seen a number of wrapper types in this post, and have said that every 
// computation expression must have an associated wrapper type.
// But what about the logging example in the previous post? There was no wrapper type 
// there. There was a let! that did things behind the scenes, but the input type was the 
// same as the output type. The type was left unchanged.
// The short answer to this is that you can treat any type as its own “wrapper”. But there 
// is another, deeper way to understand this.
// Let’s step back and consider what a wrapper type definition like List<T> really means.
// If you have a type such as List<T>, it is in fact not a “real” type at all. List<int> 
// is a real type, and List<string> is a real type. But List<T> on its own is incomplete. 
// It is missing the parameter it needs to become a real type.
// One way to think about List<T> is that it is a function, not a type. It is a function 
// in the abstract world of types, rather than the concrete world of normal values, but 
// just like any function it maps values to other values, except in this case, the input 
// values are types (say int or string) and the output values are other types (List<int> 
// and List<string>). And like any function it takes a parameter, in this case a “type 
// parameter”. Which is why the concept that .NET developers call “generics” is known as 
...
// “type constructors”), we can see that what we really mean by a “wrapper type” is just a 
// type constructor.
// But if a “wrapper type” is just a function that maps one type to another type, surely a 
// function that maps a type to the same type fits into this category? And indeed it does. 
// The “identity” function for types fits our definition and can be used as a wrapper type 
// for computation expressions.
// Going back to some real code then, we can define the “identity workflow” as the 
// simplest possible implementation of a workflow builder.
...
// With this in place, you can see that the logging example discussed earlier is just the 
// identity workflow with some logging added in.

// Summary
// Another long post, and we covered a lot of topics, but I hope that the role of wrapper 
// types is now clearer. We will see how the wrapper types can be used in practice when we 
// come to look at common workflows such as the “writer workflow” and the “state workflow” 
...
// - You can easily compose computation expressions, because the output of a Return can be 
//   fed to the input of a Bind.
