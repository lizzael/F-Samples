module _80_Computation_expressions_and_wrapper_types
// Using types to assist the workflow

// In the previous post, we were introduced to the “maybe” workflow, which allowed us to 
// hide the messiness of chaining together option types.
...
// - In the let! lines, the expression on the right of the equals is an int option, but 
//   the value on the left is just an int. The let! has “unwrapped” the option before 
//   binding it to the value.
// - And in the return line, the opposite occurs. The expression being returned is an int, 
//   but the value of the whole workflow (result) is an int option. That is, the return 
//   has “wrapped” the raw value back into an option.
// We will follow up these observations in this post, and we will see that this leads to 
// one of the major uses of computation expressions: namely, to implicitly unwrap and 
// rewrap values that are stored in some sort of wrapper type.
...
// We then use this type in our database access methods. Here are some very simple stubs 
// to give you an idea of how the DbResult type might be used:
...
// Now let’s say we want to chain these calls together. First get the customer id from the 
// name, and then get the order for the customer id, and then get the product from the 
// order.
// Here’s the most explicit way of doing it. As you can see, we have to have pattern 
...
// Really ugly code. And the top-level flow has been submerged in the error handling 
// logic.
...
// So now we have seen two workflows (the maybe workflow and the dbresult workflow), each 
// with their own corresponding wrapper type (Option<T> and DbResult<T> respectively).
// These are not just special cases. In fact, every computation expression must have an 
// associated wrapper type. And the wrapper type is often designed specifically to go 
// hand-in-hand with the workflow that we want to manage.
// The example above demonstrates this clearly. The DbResult type we created is more than 
// just a simple type for return values; it actually has a critical role in the workflow 
// by “storing” the current state of the workflow, and whether it is succeeding or failing 
// at each step. By using the various cases of the type itself, the dbresult workflow can 
// manage the transitions for us, hiding them from view and enabling us to focus on the 
// big picture.
// We’ll learn how to design a good wrapper type later in the series, but first let’s look 
// at how they are manipulated.

// Bind and Return and wrapper types
// Let’s look again at the definition of the Bind and Return methods of a computation 
// expression.
// We’ll start off with the easy one, Return. The signature of Return as documented on 
// MSDN 
...
// Note: In signatures, the wrapper type is normally called M, so M<int> is the wrapper 
// type applied to int and M<string> is the wrapper type applied to string, and so on.
// And we’ve seen two examples of this usage. The maybe workflow returns a Some, which is 
// an option type, and the dbresult workflow returns Success, which is part of the 
// DbResult type.
...
// and returns a M<'U>, where M<'U> means the wrapper type applied to type U.
...
// - Then, optionally apply the function to the unwrapped value to create a new wrapped 
//   value.
...
// types and functions:
// - For Bind, we start with a wrapped value (m here), unwrap it to a raw value of type T, 
//   and then (maybe) apply the function f to it to get a wrapped value of type U.
// - For Return, we start with a value (x here), and simply wrap it.

// The type wrapper is generic
// Note that all the functions use generic types (T and U) other than the wrapper type 
// itself, which must be the same throughout. For example, there is nothing stopping the 
// maybe binding function from taking an int and returning a Option<string>, or taking a 
// string and then returning an Option<bool>. The only requirement is that it always 
// return an Option<something>.
// To see this, we can revisit the example above, but rather than using strings 
// everywhere, we will create special types for the customer id, order id, and product id. 
...
// - More subtly, there seems to be unnecessary code in the error lines. Why 
//   write | Error e -> Error e? The reason is that the incoming error that is being 
//   matched against is of type DbResult<CustomerId> or DbResult<OrderId>, but the return 
//   value must be of type DbResult<ProductId>. So, even though the two Errors look the 
//   same, they are actually of different types.
...
// (DbResult<CustomerId>,DbResult<OrderId>, etc), but because they have the same wrapper 
// type in common, the bind works as expected.
...
// We’ve seen that every computation expression must have an associated wrapper type. This 
// wrapper type is used in both Bind and Return, which leads to a key benefit:
// - the output of a Return can be fed to the input of a Bind
// - In other words, because a workflow returns a wrapper type, and because let! consumes 
//   a wrapper type, you can put a “child” workflow on the right hand side of a let! 
//   expression.
// Y ou can write the following:
type Workflow() =
    member this.Bind(m, f) = 
        match m with
        | None -> None
...
// return it directly. return is no good for this, because it requires an unwrapped type 
// as input.
...
// implementation just returns the wrapped type “as is” (although of course, you can 
// always add extra logic behind the scenes).
...
// Summary
// This post introduced wrapper types and how they related to Bind, Return and ReturnFrom, 
// the core methods of any builder class.
// In the next post, we’ll continue to look at wrapper types, including using lists as 
// wrapper types.