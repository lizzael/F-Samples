module _32_Functional_Reactive_Programming
// Turning events into streams

// Events are everywhere. Almost every program has to handle events, whether it be button 
// clicks in the user interface, listening to sockets in a server, or even a system 
// shutdown notification.
// And events are the basis of one of the most common OO design patterns: the "Observer" 
// pattern.
// But as we know, event handling, like concurrency in general, can be tricky to implement. 
// Simple event logic is straightforward, but what about logic like "do something if two 
// events happen in a row but do something different if only one event happens" or "do 
// something if two events happen at roughly the same time". And how easy is it to combine 
// these requirements in other, more complex ways?
// Even you can successfully implement these requirements, the code tends to be spaghetti 
// like and hard to understand, even with the best intentions.
// Is there a approach that can make event handling easier?
// We saw in the previous post on message queues that one of the advantages of that 
// approach was that the requests were "serialized" making it conceptually easier to deal 
// with.
// There is a similar approach that can be used with events. The idea is to turn a series 
// of events into an "event stream". Event streams then become quite like IEnumerables, 
// and so the obvious next step is to treat them in much the way that LINQ handles 
// collections, so that they can be filtered, mapped, split and combined.
// F# has built in support for this model, as well as for the more tradition approach.

// A simple event stream
// Let's start with a simple example to compare the two approaches. We'll implement the 
// classic event handler approach first.
...
// Now let's create a similar utility method to create a timer, but this time it will 
// return an "observable" as well, which is the stream of events.
...
// Here we see how you can build up layers of event transformations, just as you do with 
// list transformations in LINQ.
// The first transformation is scan, which accumulates state for each event. It is roughly 
// equivalent to the List.fold function that we have seen used with lists. In this case, 
// the accumulated state is just a counter.
// And then, for each event, the count is printed out.
// Note that in this functional approach, we didn't have any mutable state, and we didn't 
// need to create any special classes.
...
// Create two timers, called '3' and '5'. The '3' timer ticks every 300ms and the '5' 
...
// And then we need a utility function to see if two events are simultaneous. We'll be 
// generous and allow a time difference of up to 50ms.
...
// In the imperative design, we'll need to keep track of the previous event, so we can 
// compare them. And we'll need special case code for the first time, when the previous 
// event doesn't exist
type ImperativeFizzBuzzHandler() =
    ...
    let printEvent thisEvent  = 
      ...
      let simultaneous = 
        previousEvent.IsSome && areSimultaneous (previousEvent.Value,thisEvent)
...
// Let's test it:
// create the class
let handler1 = new ImperativeFizzBuzzHandler()

// create the two timers and register the two handlers
let timer3 = createTimer 300 handler1.handleEvent3
let timer5 = createTimer 500 handler1.handleEvent5
...
// The problem with this imperative code is that it has a lot of noise that obscures the 
// the requirements.
// Can the functional version do better? Let's see!
// First, we create two event streams, one for each timer:
let timer3_1, timerEventStream3 = createTimerAndObservable 300
let timer5_1, timerEventStream5 = createTimerAndObservable 500
...
// Now, to see if two events are simultaneous, we need to compare them from the two 
// different streams somehow.
...
// The code might seem a bit long winded, but this kind of incremental, step-wise approach 
// is very clear and self-documenting.
// Some of the benefits of this style are:
// - I can see that it meets the requirements just by looking at it, without even running 
//   it. Not so with the imperative version.
// - From a design point of view, each final "output" stream follows the single 
//   responsibility principle -- it only does one thing -- so it is very easy to associate 
//   behavior with it.
...
// touched on it here. I hope this introduction has given you a glimpse of the usefulness 
// of this way of doing things.
// If you want to learn more, see the documentation for the F# Observable module
...
// Extensions (Rx) 
// http://msdn.microsoft.com/en-us/library/hh242985%28v=vs.103%29
