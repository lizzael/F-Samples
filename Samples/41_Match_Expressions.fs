module _41_Match_Expressions
// The workhorse of F#
// Pattern matching is ubiquitous in F#. It is used for binding values to expressions with 
// let, and in function parameters, and for branching using the match..with syntax.

// What is a match expression?
// We have already seen match..with expressions a number of times. And we know that it has 
// the form:
...
// This is where the patterns come in. The choice is made based on whether the "match 
// with" value can be matched with the parameter of the lambda expression. The first 
// lambda whose parameter can be made to match the input value "wins"!
// So for example, if the param is the wildcard _, it will always match, and if first, 
// always win.
// _ -> expression  
...
// On the other hand, if we changed the order to put the wildcard first, it would be tried 
// first and always win immediately:
let x2 = 
    match 1 with 
    | _ -> "z" 
    | 1 -> "a"
    | 2 -> "b"  
// In this case, the F# compiler helpfully warns us that the other rules will never be 
// matched.
...
// Since F# is sensitive to indentation, you might be wondering how best to format this 
// expression, as there are quite a few moving parts.
// The post on F# syntax gives an overview of how alignment works, but for match..with 
// expressions, here are some specific guidelines.
...
// The match..with can be on the same line or a new line, but using a new line keeps the 
// indenting consistent, independent of the lengths of the names:
                                              // ugly alignment!  
let myVeryLongNameForAFunction1 myParameter =  match myParameter with 
                                              | 1 -> "something" 
                                              | _ -> "anything" 

// much better
let myVeryLongNameForAFunction2 myParameter =  
...
// again keeps the indenting consistent and helps to separate the match pattern from the 
// result expression.
let f1 x =  
    match x with 
...
// It is important to realize that match..with is not really a "control flow" construct. 
// The "control" does not "flow" down the branches, but instead, the whole thing is an 
// expression that gets evaluated at some point, just like any other expression. The end 
// result in practice might be the same, but it is a conceptual difference that can be 
// important.
// One consequence of it being an expression is that all branches must evaluate to the 
// same type -- we have already seen this same behavior with if-then-else expressions and 
// for loops.
// let x = 
//    match 1 with 
//    | 1 -> 42
...
// Since they are normal expressions, match expressions can appear anywhere an expression 
// can be used.
...
// Exhaustive matching
// Another consequence of being an expression is that there must always be some branch 
// that matches. The expression as a whole must evaluate to something!
// That is, the valuable concept of "exhaustive matching" comes from the 
// "everything-is-an-expression" nature of F#. In a statement oriented language, there 
// would be no requirement for this to happen.
...
// The compiler will warn you if it thinks there is a missing branch. And if you 
// deliberately ignore the warning, then you will get a nasty runtime error 
// (MatchFailureException) when none of the patterns match.

// Exhaustive matching is not perfect
// The algorithm for checking that all possible matches are listed is good but not always 
// perfect. Occasionally it will complain that you have not matched every possible case, 
// when you know that you have. In this case, you may need to add an extra case just to 
// keep the compiler happy.

// Using (and avoiding) the wildcard match
// One way to guarantee that you always match all cases is to put the wildcard parameter 
// as the last match:
let x4 = 
    match 42 with 
    | 1 -> "a"
...
// You see this pattern frequently, and I have used it a lot in these examples. It's the 
// equivalent of having a catch-all default in a switch statement.
// But if you want to get the full benefits of exhaustive pattern matching, I would 
// encourage you not to use wildcards, and try to match all the cases explicitly if you 
// can. This is particularly true if you are matching on the cases of a union type:
type Choices = A | B | C
let x5 = 
    match A with 
    | A -> "a"
...
// By being always explicit in this way, you can trap any error caused by adding a new 
// case to the union. If you had a wildcard match, you would never know.
// If you can't have every case be explicit, you might try to document your boundary 
...
// By the way, I have deliberately left this pattern (and others in this post) as 
// incomplete. As an exercise, make them complete without using the wildcard.
// It is important to note that the values that are bound must be distinct for each 
// pattern. So you can't do something like this:
...
// This second option can also be rewritten using "guards" (when clauses) instead. Guards 
...
// It is important to understand that sequences (aka IEnumerables) can not be matched on 
// this way directly, because they are "lazy" and meant to be accessed one element at a 
// time. Lists and arrays, on the other hand, are fully available to be matched on.
// Of these patterns, the most common one is the "cons" pattern, often used in conjunction 
// with recursion to loop through the elements of the list.
...
// The second example shows how we can carry state from one iteration of the loop to the 
// next using a special "accumulator" parameter (called sumSoFar in this example). This is 
...
// This only works to find subclasses of a parent class (in this case, Object). The 
// overall type of the expression has the parent class as input.
...
// In my opinion, matching and dispatching on types is a code smell, just as it is in 
// object-oriented programming. It is occasionally necessary, but used carelessly is an 
...
// All the patterns we've looked at so far do pattern matching on a single value. How can 
// you do it for two or more?
// The short answer is: you can't. Matches are only allowed on single values.
// But wait a minute -- could we combine two values into a single tuple on the fly and 
// match on that? Yes, we can!
...
// And indeed, this trick will work whenever you want to match on a set of values -- just 
// group them all into a single tuple.
...
// conditional tests.
// But there is a way to do the equality test as part of the pattern match -- using an 
// additional when clause to the left of the function arrow. These clauses are known as 
// "guards".
// Here's the same logic written using a guard instead:
let elementsAreEqual2 aTuple = 
    match aTuple with 
    | (x,y) when x=y -> 
        printfn "both parts are the same" 
    | _ ->
...
// This is nicer, because we have integrated the test into the pattern proper, rather than 
// using a test after the match has been done.
// Guards can be used for all sorts of things that pure patterns can't be used for, such 
// as:
...
// Guards are great for one-off matches. But if there are certain guards that you use over 
// and over, consider using active patterns instead.
...
// In the special case of function definitions we can simplify this dramatically by using 
// the function keyword.
let f0 = 
    function 
    | _ -> "something" 
...
// match..with.
// This keyword is not the same as the fun keyword for standard lambdas, rather it 
// combines fun and match..with in a single step.
...
// (with the use of as as well).
// Because the try..with expression implements full pattern matching, we can also use 
// guards as well, if needed to add extra conditional logic:
let debugMode = false
try
    failwith "fail"
with
...
// The main problem is that match expressions doesn't compose very well. That is, it is 
// hard to chain match..with expressions and build simple ones into complex ones.
// The best way of avoiding this is to wrap match..with expressions into functions, which 
// can then be composed nicely.
// Here's a simple example. The match x with 42 is wrapped in a isAnswerToEverything 
// function.
let times6 x = x * 6
...
// For example, instead of using recursion to loop through lists, you should try to use 
// the functions in the List module, which will do almost everything you need.
...
// Finally, if you create your own types which need to be frequently matched, it is good 
// practice to create a corresponding generic "fold" function that wraps it nicely.
// For example, here is a type for defining temperature.
type TemperatureType  = F of float | C of float

// Chances are, we will matching these cases a lot, so let's create a generic function 
...
// - there is one function for each case in the union structure (or clause in the 
//   match pattern)
// - finally, the actual value to match on comes last.
// Now we have our fold function, we can use it in different contexts.
// Let's start by testing for a fever. We need a function for testing degrees F for fever 
// and another one for testing degrees C for fever.
...
// Note that the conversion functions wrap the converted values in a new TemperatureType, 
...
// We can even call convert twice in a row, and we should get back the same temperature 
