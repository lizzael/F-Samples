module _42_Formatted_text_using_printf
// Tips and techniques for printing and logging
// In this post, we'll take a small detour and look at how to create formatted text. The 
// printing and formatting functions are technically library functions, but in practice 
// they as used as if they were part of the core language.
// F# supports two distinct styles of formatting text:
// - The standard .NET technique of "composite formatting" as seen in String.Format, 
//   Console.WriteLine and other places.
...
// The composite formatting technique is available in all .NET languages, and you are 
// probably familiar with it from C#.
...
// Unlike String.Format, printf is statically type checked, both for the types of the 
...
// The equivalent code using composite formatting will compile fine but either work 
// incorrectly but silently, or give a runtime error:
// wrong parameter type
...
// For non-primitive types, the .NET formatting functions only support using ToString(), 
...
// so if you want to use them with the .NET formatting functions, you will have to 
// override the ToString() method explicitly.

// printf gotchas
// There are a couple of "gotchas" to be aware of when using printf.
// First, if there are too few parameters, rather than too many, the compiler will not 
// complain immediately, but might give cryptic errors later.
// too few parameters
printfn "A string: %s An int: %i" "Hello" 

// The reason, of course, is that this is not an error at all; printf is just being 
// partially applied! See the discussion of partial application if you are not clear of 
// why this happens.
// Another issue is that the "format strings" are not actually strings.
// In the .NET formatting model, the formatting strings are normal strings, so you can 
// pass them around, store them in resource files, and so on. Which means that the 
// following code works fine:
let netFormatString = "A string: {0}"
// Console.WriteLine(netFormatString, "hello")

// On the other hand, the "format strings" that are the first argument to printf are not 
// really strings at all, but something called a TextWriterFormat. Which means that the 
// following code does not work:
// let fsharpFormatString = "A string: %s"
// printfn fsharpFormatString  "Hello" 
// The compiler does some magic behind the scenes to convert the string constant 
// "A string: %s" into the appropriate TextWriterFormat. The TextWriterFormat is the key 
// component that "knows" the type of the format string, such as string->unit or 
// string->int->unit, which in turn allows printf to be typesafe.
// If you want to emulate the compiler, you can create your own TextWriterFormat value 
// from a string using the Printf.TextWriterFormat type in the 
// Microsoft.FSharp.Core.Printf module.
// If the format string is "inline", the compiler can deduce the type for you during 
// binding:
let format:Printf.TextWriterFormat<_> = "A string: %s"
printfn format "Hello" 

// But if the format string is truly dynamic (e.g. stored in a resource or created on the 
// fly), the compiler cannot deduce the type for you, and you must explicitly provide it 
// with the constructor.
// In the example below, my first format string has a single string parameter and returns 
// a unit, so I have to specify string->unit as the format type. And in the second case, I 
// have to specify string->int->unit as the format type.
...
// I won't go into detail on exactly how printf andTextWriterFormat` work together right 
// now -- just be aware that is not just a matter of simple format strings being passed 
// around.
// Finally, it's worth noting that printf and family are not thread-safe, while 
// Console.Write and family are.

// How to specify a format
// The "%" format specifications are quite similar to those used in C, but with some 
// special customizations for F#.
// As with C, the characters immediately following the % have a specific meaning, as shown 
// below.
...
// When formatting fixed width columns and tables, you need to have control of the 
// alignment and width.
// You can do that with the "width" and "flags" options.
// - %5s, %5i. A number sets the width of the value
// - %*s, %*i. A star sets the width of the value dynamically (from an extra parameter 
//   just before the param to format)
...
// The specifiers do not enforce any type safety within the integer types. As you can see 
// from the examples above, you can pass a signed int to an unsigned specifier without 
// problems. What is different is how it is formatted. The unsigned specifiers treat the 
// int as unsigned no matter how it is actually typed.
...
// Here are some examples:
let rows2 = [ (1,"a"); (-22,"bb"); (333,"ccc"); (-4444,"dddd") ] 
...
// The decimal type can be used with the floating point specifiers, but you might lose 
// some precision. The %M specifier can be used to ensure that no precision is lost. You 
// can see the difference with this example:
let largeM = 123456789.123456789M  // a decimal
printfn "float: %f decimal: %M" largeM largeM 

// You can control the precision of floats using a precision specification, such as %.2f 
// and %.4f. For the %f and %e specifiers, the precision affects the number of digits 
// after the decimal point, while for %g it is the number of digits in total. Here's an 
// example:
...
// There are two special format specifiers that allow to you pass in a function rather 
...
// Obviously, since the callback function takes no parameters, it will probably be a 
// closure that does reference some other value. Here's an example that prints random 
// numbers:
open System
open System.IO
...
// For the %a specifier, the callback function takes an extra parameter. That is, when 
// using the %a specifier, you must pass in both a function and a value to format.
...
// The ToString with %s is easier to test and use, but it will be less efficient than 
// writing directly to a TextWriter.

// The printf family of functions
// There are a number of variants of printf functions. Here is a quick guide:
// - printf and printfn: (Console.Write and Console.WriteLine) Functions starting with 
//   "print" write to standard out.
// - eprintf and eprintfn: (Console.Error.Write and Console.Error.WriteLine) Functions 
//   starting with "eprint" write to standard error.
// - fprintf and fprintfn: (TextWriter.Write and TextWriter.WriteLine) Functions starting 
//   with "fprint" write to a TextWriter.
// - sprintf: (String.Format) Functions starting with "sprint" return a string.
// - bprintf: (StringBuilder.AppendFormat) Functions starting with "bprint" write to a 
//   StringBuilder.
// - kprintf, kfprintf, ksprintf and kbprintf: (No equivalent) Functions that accept a 
//   continuation. See next section for a discussion.
...
// A particularly useful technique is to use partial application to "bake in" a 
...
// Note that in both cases above, we had to pass a format parameter when creating the 
// partial application.
// That is, we had to do:
// let myPrint format = fprintf sw format
// rather than the point-free version:
// let myPrint  = fprintf sw 
// This stops the compiler complaining about an incorrect type. The reason why is 
// non-obvious. We briefly mentioned the TextWriterFormat above as the first parameter to 
// printf. It turns out that printf is not actually a particular function, like 
// String.Format, but rather a generic function that has to be parameterized with a 
// TextWriterFormat (or the similar StringFormat) in order to become "real".
// So, to be safe, it is best to always pair a printf with a format parameter, rather than 
// being overly aggressive with the partial application.

// The kprintf functions
// The four kXXX functions are similar to their cousins, except that they take an extra 
// parameter -- a continuation. That is, a function to be called immediately after the 
...
// - You can pass the result to another function that does something useful, such as a 
//   logging framework
...
// First, let's create a simple logging class along the lines of log4net or 
// System.Diagnostics.Trace. In practice, this would be replaced by a real third-party 
// library.
open System
open System.IO
...
// - Create an instance of the logging framework. I've hard-coded the factory method here, 
//   but you could also use an IoC container.
// - Create helper functions called logInfo and logError that call the logging framework, 
//   and in the case of logError, show a popup message as well.
// my application code
...
// Finally, when we run the test function, we should get the message written to the 
// console, and also see the popup message:
MyApplication.test()
...
// The object-oriented approach, although more familiar, is not automatically better!