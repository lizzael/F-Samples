module _40_Exceptions
// Syntax for throwing and catching

// Just like other .NET languages, F# supports throwing and catching exceptions. As with 
// the control flow expressions, the syntax will feel familiar, but again there are a few 
// catches that you should know about.

// Defining your own exceptions
// When raising/throwing exceptions, you can use the standard system ones such as 
// InvalidOperationException, or you can define your own exception types using the simple 
...
// We said earlier that both branches of an if-then-else expression must return the same 
// type. But how can raising an exception work with this constraint?
// The answer is that any code that raises exceptions is ignored for the purposes of 
// determining expression types. This means that the function signature will be based on 
// the normal case only, not the exception case.
// For example, in the code below, the exceptions are ignored, and the overall function 
// has signature bool->int, as you would expect.
let f8 x = 
   if x then 42
   elif true then failwith "message"
   else invalidArg "paramName" "message"   

// Question: what do you think the function signature will be if both branches raise 
// exceptions?
let f9 x = 
...
// Catching exceptions
// Exceptions are caught using a try-catch block, as in other languages. F# calls it 
// try-with instead, and testing for each type of exception uses the standard pattern 
// matching syntax.
try
    failwith "fail"
with
...
// If the exception to catch was thrown with failwith (e.g. a System.Exception) or a 
// custom F# exception, you can match using the simple tag approach shown above.
// On the other hand, to catch a specific .NET exception class, you have to match using 
// the more complicated syntax:
// :? (exception class) as ex 
// Again, as with if-then-else and the loops, the try-with block is an expression that 
// returns a value. This means that all branches of the try-with expression must return 
// the same type.
...
// To fix this, we need to make the "with" branch also return type int. We can do this 
// easily using the semicolon trick to chain expressions on one line.
let divide x y=
    try
        (x+1) / y                      
    with
...
// Now that the try-with expression has a defined type, the whole function can be assigned 
// a type, namely int -> int -> int, as expected.
// As before, if any branch throws an exception, it doesn’t count when types are being 
// determined.

// Rethrowing exceptions
// If needed, you can call the "reraise()" function in a catch handler to propagate the 
// same exception up the call chain. This is the same as the C# throw keyword.
let divide2 x y=
    try
        (x+1) / y                      
    with
...
// Try-finally
// Another familiar expression is try-finally. As you might expect, the "finally" clause 
...
// The return type of the try-finally expression as a whole is always the same as return 
// type of the "try" clause on its own. The "finally" clause has no effect on the type of 
// the expression as a whole. So in the above example, the whole expression has type 
// string.
// The "finally" clause must always return unit, so any non-unit values will be flagged by 
// the compiler.
...
// The try-with and the try-finally expressions are distinct and cannot be combined 
...
// When you are designing a function, should you throw exceptions, or return structures 
// which encode the error? This section will discuss two different approaches.
// The pair of functions approach
// One approach is to provide two functions: one which assumes everything works and 
// throws an exception otherwise and a second "tryXXX" function that returns a missing 
// value if something goes wrong.
// For example, we might want to design two distinct library functions for division, one 
// that doesn't handle exceptions and one that does:
...
// Note the use of Some and None Option types in the tryDivide code to signal to the 
// client whether the value is valid.
...
// Note that there is no constraint that forces the client to do this, so this approach 
// can be a source of errors.
// With the second function the client code is simpler, and the client is constrained to 
// handle both the normal case and the error case.
...
// Note that I prefer to have an "Exn" suffix on "doSomething" rather than no suffix at 
// all. It makes it clear that you expect clients to catch exceptions even in normal cases.
// The overall problem with this approach is that you have to do extra work to create 
// pairs of functions, and you reduce the safety of the system by relying on the client to 
// catch exceptions if they use the unsafe version of the function.

// The error-code-based approach
// "Writing good error-code-based code is hard, but writing good exception-based code is 
// really hard." Raymond Chen
// In the functional world, returning error codes (or rather error types) is generally 
// preferred to throwing exceptions, and so a standard hybrid approach is to encode the 
// common cases (the ones that you would expect a user to care about) into a error type, 
// but leave the very unusual exceptions alone.

// Often, the simplest approach is just to use the option type: Some for success and None 
// for errors. If the error case is obvious, as in tryDivide or tryParse, there is no need 
// to be explicit with more detailed error cases.
// But sometimes there is more than one possible error, and each should be handled 
// differently. In this case, a union type with a case for each error is useful.
// In the following example, we want to execute a SqlCommand. Three very common error 
// cases are login errors, constraint errors and foreign key errors, so we build them into 
// the result structure. All other errors are raised as exceptions.
...
// Unlike a traditional error code approach, the caller of the function does not have to 
// handle any errors immediately, and can simply pass the structure around until it gets 
// to someone who knows how to handle it, as shown below:
...
// On the other hand, unlike C#, the result of a expression cannot be accidentally thrown 
// away. So if a function returns an error result, the caller must handle it (unless it 
// really wants to be badly behaved and send it to ignore)

let presentationLayerFunction2 = 
